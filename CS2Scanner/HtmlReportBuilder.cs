...
namespace CS2Scanner
{
    public static class HtmlReportBuilder
    {
        public static string Build(ReportData data)
        {
            ...
            string drsRows = BuildDrsRows(data.NvidiaDrsEntries);
        ...
        private static string BuildDrsRows(IReadOnlyList<NvidiaDrsEntry> entries)
        {
            if (entries.Count == 0)
            {
                return "<tr><td colspan=\"2\">Данные отсутствуют</td></tr>";
            }
            ...
            {
                sb.Append("<tr>");
                sb.Append("<td>").Append(WebUtility.HtmlEncode(entry.ProfileOrKey)).Append("</td>");
